...
def start_benchmark(args, model, image, trt_model_path, tensor_db, np_db):
    ...
    if args.compile:
        ...
        engine = backend.prepare(model, device='CUDA:' + str(torch.cuda.current_device()))
...
def main(args):
    ...
    if args.eval_only:
        ...
        state = pickle.load(open('model_final_cc1b1f.pkl', 'rb'))
        ...
        dataset_as_tensor, dataset_as_np = prepare_dataset("/mnt/sda/datasets/cityscapes/cityscapes/leftImg8bit/test/bielefeld/", "cpu", args.sample_factor)
